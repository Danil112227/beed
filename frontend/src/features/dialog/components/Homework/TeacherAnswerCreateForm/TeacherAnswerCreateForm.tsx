...
import { TeacherAnswerCreateFormProps } from "./types";

import Upload from "@assets/vectors/upload.svg?react";

import "./TeacherAnswerCreateForm.styles.scss";

function TeacherAnswerCreateForm({
	isVisible,
	answerId,
}: TeacherAnswerCreateFormProps) {
	const { mutate: createHomeworkTeacherAnswerMutation } = useMutation<
		CreateHomeworkTeacherAnswerSuccessResponse,
		CreateHomeworkTeacherAnswerValidationError,
		CreateHomeworkTeacherAnswer
	>({
		mutationFn: createHomeworkTeacherAnswer,
	});
	...
	useEffect(() => {
		if (answerId && isVisible) {
			setValue("answer", answerId);
		}
	}, [answerId, isVisible, setValue]);
	...
	const submitFormHandler = (data: CreateHomeworkTeacherAnswerFields) => {
		createHomeworkTeacherAnswerMutation(
			{ answer: data },
			{
				onError(error) {
					...
					for (const key in errors) {
						if (Object.prototype.hasOwnProperty.call(errors, key)) {
							const errorKey =
								key as keyof CreateHomeworkTeacherAnswerValidationError["data"];
...
export { TeacherAnswerCreateForm };
