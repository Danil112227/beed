...
import { StudentAnswerEditFormProps } from "./types";

import Upload from "@assets/vectors/upload.svg?react";

import "./StudentAnswerEditForm.styles.scss";

function StudentAnswerEditForm({
	isVisible,
	homeworkId,
	studentAnswer,
	onClose,
}: StudentAnswerEditFormProps) {
	const { id } = useParams();

	const { user } = useAuth({});

	const { mutate: updateHomeworkStudentAnswerMutation } = useMutation<
		UpdateHomeworkStudentAnswerSuccessResponse,
		UpdateHomeworkStudentAnswerValidationError,
		UpdateHomeworkStudentAnswer
	>({
		mutationFn: updateHomeworkStudentAnswer,
	});
	...
	const submitFormHandler = (data: CreateHomeworkStudentAnswerFields) => {
		...
		updateHomeworkStudentAnswerMutation(
			{ answerId: studentAnswerId.toString(), answer: data, userId },
			{
				onError(error) {
					...
					for (const key in errors) {
						if (Object.prototype.hasOwnProperty.call(errors, key)) {
							const errorKey =
								key as keyof UpdateHomeworkStudentAnswerValidationError["data"];
